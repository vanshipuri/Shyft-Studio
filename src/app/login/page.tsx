...
export default function LoginPage() {
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-100 via-brand-50 to-brand-200 px-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 p-10 md:p-12">
        <div className="mb-8">
        ...
        <form action="/api/auth/login" method="POST" className="space-y-4">
          ...
          <button type="submit" className="w-full rounded-xl bg-gradient-to-tr from-brand-900 to-brand-800 text-white font-bold py-3.5 shadow-xl shadow-brand-900/20 hover:shadow-2xl hover:-translate-y-0.5 transition duration-200">Sign in</button>
        </form>
        ...
        <div className="mt-8 pt-6 border-t border-brand-200/60">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            <button type="button" onClick={() => { const e = document.getElementById("email") as HTMLInputElement; const p = document.getElementById("password") as HTMLInputElement; if(e && p){ e.value = "samyak@shyft.studio"; p.value = "password123"; } }} className="text-left px-2.5 py-2 rounded-lg bg-brand-50 border border-brand-200 hover:border-amber hover:bg-amber-soft transition text-brand-700 font-medium truncate" title="samyak@shyft.studio">Owner</button>
            <button type="button" onClick={() => { const e = document.getElementById("email") as HTMLInputElement; const p = document.getElementById("password") as HTMLInputElement; if(e && p){ e.value = "abhishek@shyft.studio"; p.value = "password123"; } }} className="text-left px-2.5 py-2 rounded-lg bg-brand-50 border border-brand-200 hover:border-amber hover:bg-amber-soft transition text-brand-700 font-medium truncate" title="abhishek@shyft.studio">Sales</button>
            <button type="button" onClick={() => { const e = document.getElementById("email") as HTMLInputElement; const p = document.getElementById("password") as HTMLInputElement; if(e && p){ e.value = "siddhant@shyft.studio"; p.value = "password123"; } }} className="text-left px-2.5 py-2 rounded-lg bg-brand-50 border border-brand-200 hover:border-amber hover:bg-amber-soft transition text-brand-700 font-medium truncate" title="siddhant@shyft.studio">Production</button>
          </div>
